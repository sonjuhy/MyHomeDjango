...
from core.models import FilePrivate, FilePublic, FilePublicTrashTb, FilePrivateTrashTb, FileDefaultPathTb
...
class DBConnection:
    ...
    def main_query(self, mode: str, data: Any) -> List[Any]:
    ...
    def get_default_path(self, mode: str) -> List[str]:
    ...
    def remove_to_trash_query(self, data: Dict[Any, Any]) -> None:
        ...
        if column.TYPE_CHAR == self.dir_type:
            ...
            for tmp_dir in dirs:
                ...
                uuid_str = str(uuid.uuid3(uuid.NAMESPACE_DNS, tmp_path))
                ...
                if tmp_dir.TYPE_CHAR == self.dir_type:
                    ...
                    if data[dataType.TYPE] == self.public_type:
                        ...
                        new_trash_dto = FilePrivateTrashTb(  # type: ignore
    ...
    def restore_from_trash_query(self, data: Dict[Any, Any]) -> None:
        column = self.schema.objects.get(uuid_char=data[dataType.UUID])
        if column.type_char == self.dir_type:
            ...
            for tmp_dir in tmp_dirs:
                uuid_str = str(uuid.uuid3(uuid.NAMESPACE_DNS, tmp_dir.origin_path_char))
                if tmp_dir.type_char == self.dir_type:
                    type_str = self.dir_type
                    size_float = 0
                    state_int = 0
    ...
    def move_query(self, data: Dict[Any, Any]) -> None:
        column = self.schema.objects.get(UUID_CHAR=data[dataType.UUID])
    ...
    def delete_query(self, uuid_str: str) -> None:
        ...
        if column.TYPE_CHAR == self.dir_type:
            ...
            logger.info('delete_query result : {}'.format(delete_result))
