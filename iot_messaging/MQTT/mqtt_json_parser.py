...
def json_parser_from_switch(msg: str) -> Dict[str, str]:  # payload to dictionary from switch
    json_object = json.loads(msg)
    dic = {'sender': json_object['sender'], 'message': json_object['message'], 'room': json_object['room']}
    return dic


def json_parser_from_else(msg: str) -> Dict[str, str]:  # payload to dictionary from else(android or server)
    json_object = json.loads(msg)
...
def json_parser_from_job(msg: str) -> Dict[str, str]:
    json_object = json.loads(msg)
    dic = {'pk': json_object['pk'], 'activation': json_object['activation']}
    return dic


def json_encode_to_switch(dic: Dict[str, str]) -> str:
...
def json_encode_to_android(dic: Dict[str, str]) -> str:
...
def json_encode_to_server(dic: Dict[str, str]) -> str:
